...
这是《气闸孤注：γ-7》的权威 WebSocket 服务器。服务器保存真实游戏状态，并按玩家视角下发经过裁剪的状态，因此客户端永远拿不到弹序或随机状态。
...
```text
server.mjs                 《气闸孤注：γ-7》服务器入口
games/deepspace.mjs        《气闸孤注：γ-7》规则模块
...
`games/nim.mjs` 和 `multiplayer.html` 是早期通用联机示例，会保留但不属于《气闸孤注：γ-7》的运行路径。
